...
With the lid open, automatic mode first attempts to enable the panel and enforces a 60-second startup recovery window. An explicit `resume` re-arms a running daemon immediately, clearing the remaining startup grace and any reconnect latch. Automatic mode then uses CoreGraphics callbacks with a two-second safety watchdog. All changes are session-only; logout or restart should discard them. `SIGINT`, `SIGTERM`, `SIGHUP`, and `SIGQUIT` restore the built-in display before exit when the lid is open; clamshell mode defers restoration until the next open-lid session.
...
Clamshell mode remains owned by macOS. builtinctl reads `AppleClamshellState` and subscribes to root power-domain lid notifications. While the lid is closed, automatic mode performs no display mutations and defers any pending recovery. Opening the lid starts a three-second topology settling window before normal policy resumes. Manual display changes and commands that require verified restoration, including `suspend`, agent lifecycle operations, and `purge`, require the lid to be open.
